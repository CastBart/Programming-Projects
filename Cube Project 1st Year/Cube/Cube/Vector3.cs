...
namespace Lab6Starter
{
    ...
    class Vector3
    {
        ...
        public Vector3 ParralelComponent(Vector3 v2)
        {
            ...
            lenghtVec2 = (float)(Math.Sqrt((x * x) + (y * y) + (z * z)));

            firstCalc = DotProduct(v2);
            float secondCalc = lenghtVec2 * lenghtVec2;
            if (secondCalc == 0)
            {
                return new Vector3();
            }
        ...
        public static Point[] ConvertToPoints(Vector3[] vectors)
        {
        ...
        /// <summary>
        /// the method returns a boolean if a face faces towards you
        /// by getting the cross product of opposite sides.
        /// #RightHandRule #ForTheWin
        /// </summary>
        /// <param name="vector31"></param>
        /// <param name="vector32"></param>
        /// <param name="vector33"></param>
        /// <returns></returns>
        internal static bool FacingTowardsMe(Vector3 vector31, Vector3 vector32, Vector3 vector33)
        {
            bool facingMe = false;
            if (((vector31-vector32).CrossProduct((vector31-vector33))).z > 0)
            {
                facingMe = true;
            }
            return facingMe;
