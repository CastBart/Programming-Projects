...
void operator >> (const YAML::Node& pathNode, PathData& point)
{
	point.m_type = pathNode["type"].as<std::string>();
	point.m_position.x = pathNode["position"]["x"].as<float>();
	point.m_position.y = pathNode["position"]["y"].as<float>();
}
...
void operator >> (const YAML::Node& levelNode, LevelData& level)
{
   ...
   const YAML::Node& pathNode = levelNode["path"].as<YAML::Node>();
   for (unsigned i = 0; i < pathNode.size(); ++i)
   {
	   PathData points;
	   pathNode[i] >> points;
	   level.m_pathPoints.push_back(points);
   }
}
